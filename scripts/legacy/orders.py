...
class AmazonScript:
    ...
    def get_orders_from_sp_api(self) -> List[AmazonOrder]:
        ...
        return self.convert_response_to_amazon_order_list(
            res.get_orders(CreatedAfter='2023-02-23', CreatedBefore='2023-03-01').payload
            # res.get_orders(CreatedAfter='2023-03-30', CreatedBefore=date.today().isoformat()).payload
        )
    ...
    def convert_response_to_inventory_list(
            response_payload: dict
    ) -> List[AmazonOrder]:
        amazon_order_list = []
        for item in response_payload.get("Inventory"):
            amazon_order_list.append(
                AmazonOrder(
                    order_id=item.get("AmazonOrderId", ''),
                    purchase_date=item.get("PurchaseDate", ''),
